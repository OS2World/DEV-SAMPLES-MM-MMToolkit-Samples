...
#include <string.h>
#include <stdlib.h>
#include <mmio.h>                      // public include files
...
*     WORD wMsg    - MMIO message being sent
...

{
   ...
   PSZ pszData;
   CHAR szHeaderLength[2];          // storage for header length (WORD)
   ...
   WORD wReturnCode;                // return code from mmioClose
   ...
   DWORD dwTempFlags;               // temp flags for flags to be removed.
   ...
   if (pmmioStr)
      ...
      pInfo = (PCONVPROCINFO) &pmmioinfo->adwInfo;
      pmmioinfo->dwErrorRet = MMIO_SUCCESS;
      }
   else
      {
      pmmioinfo = NULL;
   ...
   switch( wMsg )
      ...
      case MMIOM_OPEN:
         ...
         if (!pmmioinfo->fccChildIOProc)
            ...
            if (pmmioinfo->dwFlags & MMIO_CREATE)
               {
          ...
          *  3. Save the hmmcf handle in adwInfo[1] if sent in.
         ...
         memmove( &mmioinfoSS.adwInfo, pmmioinfo->adwInfo,(4*sizeof(DWORD)));
         mmioinfoSS.dwFlags = pmmioinfo->dwFlags;

         dwTempFlags = (MMIO_CREATE|MMIO_READ|MMIO_WRITE|MMIO_READWRITE|
         ...
         mmioinfoSS.dwFlags &= dwTempFlags;
         mmioinfoSS.dwFlags |= MMIO_NOIDENTIFY;

         hmmioSS = mmioOpen( pszFileName, &mmioinfoSS, mmioinfoSS.dwFlags );
         ...
         if (hmmioSS)
            ...
            if (pmmioinfo->dwFlags & MMIO_DELETE)
               {
               return (MMIO_SUCCESS);
               }
         ...
         if (!(pmmioinfo->dwFlags & MMIO_CREATE))
            ...
            pconvHeaderInfo->dwHeaderLength = sizeof(CONVHEADERINFO);
      ...
      case MMIOM_WRITE:
         ...
         if ( lBytesWritten < 0L )
            ...
            {
            pmmioinfo->dwErrorRet = MMIOERR_CANNOTWRITE;
            ...
            if (pconvHeaderInfo)
               pconvHeaderInfo->dwFlags = CONV_MODIFIED_FILE;
      ...
      case MMIOM_CLOSE:
            ...
            if (((pmmioinfo->dwFlags & MMIO_WRITE) ||
                 (pmmioinfo->dwFlags & MMIO_READWRITE)) &&
            ...
            wReturnCode = mmioClose( pInfo->hmmioSS, 0L );
            return ((ULONG)wReturnCode);
      ...
      case MMIOM_GETFORMATINFO:
         ...
         pmmformatinfo->dwStructLen = sizeof(MMFORMATINFO);
         pmmformatinfo->fccIOProc = FOURCC_FFT;
         pmmformatinfo->dwIOProcType = MMIO_IOPROC_FILEFORMAT;
         pmmformatinfo->dwMediaType = MMIO_MEDIATYPE_OTHER;
         pmmformatinfo->dwFlags = MMIO_CANREADUNTRANSLATED      |
                                  MMIO_CANWRITEUNTRANSLATED     |
                                  MMIO_CANREADWRITEUNTRANSLATED |
                                  MMIO_CANSEEKUNTRANSLATED;

         memset( pmmformatinfo->achDefaultFormatExt, '\0',
            sizeof(pmmformatinfo->achDefaultFormatExt) );
         strcpy( pmmformatinfo->achDefaultFormatExt, "FFT" );

         if (convhlpGetNLSData( &pmmformatinfo->dwCodePage,
                                &pmmformatinfo->dwLanguage ))
            {
            return( -1L );
            }
      ...
      default:
         ...
         if (pInfo->hmmioSS)
            {
            return( mmioSendMessage( pInfo->hmmioSS,
                                     wMsg,
                                     lParam1,
                                     lParam2 ));
            }
            ...
            {
            pmmioinfo->dwErrorRet = MMIOERR_UNSUPPORTED_MESSAGE;
