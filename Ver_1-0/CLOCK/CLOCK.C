 ...
 *               clock.lrf       Linker response file.
 *               clock.ico       Clock icon.
 *               bellc.bm        Center bell bitmap.
...
#define  INCL_MMIO                       /* Required for MMIO information.    */
...
RECTL rctBitMapRectangle;                            /* Area of the bitmap.   */
ULONG ulThreadId   = 1;                              /* Swing thread id.      */
...
POINTL  ptlBitMapPoint;                              /* Place Bitmaps.        */
...
USHORT  usLastHour   = 99, /* The last hour when in the Draw Hands proc.      */
        ...
        usMinute     = 61; /* Contains current minute given by UpdateTheCock. */
...
VOID InitializeDialog( VOID )
{
   ...
   ULONG    ulThreadId = 1;              /* Thread swing id.                  */
    ...
    */
   if ( DosCreateThread(
           &ulThreadId,                   /* Thread id.                       */
           (PFNTHREAD) SwingTheBell,      /* Routine to run in the thread.    */
           (ULONG) NULL,                  /* What parameters to pass.         */
           (ULONG) 0,                     /* Start the thread.                */
                                 ...
                                 MPARAM mp2 )
{
   ...
   ULONG    ulBorderXSize,          /* Size of X border.                      */
            ulBorderYSize,          /* Size of Y border.                      */
            ulPostCount;            /* Count of number of Posts for the Sem.  */
   RECTL    rctWindowRectangle;     /* The area rectangle of the window.      */
   DWORD    dwError;                /* RC for acquire device.                 */
   ...
   switch( msg )
   {
      ...
      case WM_INITDLG :
         ...
         /*
          * Set up the position of the bitmaps.  The rctBitMapRectangle is
          * used to invalidate the region of the Presentation Space that
          * contains the Bell bitmap.  The ptlBitMapPoint is used to place
          * the bitmaps.
          */
         WinQueryWindowRect(
         ...
         rctBitMapRectangle.xLeft   =
            rctWindowRectangle.xRight  - BELL_BITMAP_X_SIZE - 25;
         rctBitMapRectangle.yBottom =
            rctWindowRectangle.yBottom + Y_BITMAP_POSITION;
         rctBitMapRectangle.xRight  =
            rctBitMapRectangle.xLeft   + BELL_BITMAP_X_SIZE;
         rctBitMapRectangle.yTop    =
            rctWindowRectangle.yBottom + BELL_BITMAP_Y_SIZE + 10;

         ptlBitMapPoint.x = rctBitMapRectangle.xLeft;
         ptlBitMapPoint.y = rctBitMapRectangle.yBottom;


         /*
          * Get the presentation space for the window so that the bell
          * bitmaps can be loaded.  Once the bitmaps have been loaded the
          * presentation space can be released.
          */
         hps = WinGetPS( hwnd );
      ...
      case WM_CLOSE :
         if ( mciOpenParameters.wDeviceID > 0 )
         {
            mciSendCommand(
               mciOpenParameters.wDeviceID,  /* Device to play the chimes.    */
               MCI_CLOSE,                    /* MCI message.                  */
               MCI_WAIT,                     /* Flags for the MCI message.    */
               (DWORD) NULL,                 /* Parameters for the message.   */
               (DWORD) NULL );               /* Parameter for notify message. */
         }
      ...
      case WM_PAINT :
      {
         if ( fIsTheWindowMinimized == FALSE )
         {
             ...
             */
            if ( yngContinueToSwingTheBell == READY &&
                  fThreadIsAlive == TRUE )
            {
            ...
            if ( yngContinueToSwingTheBell == GO )
            {
               WinDrawBitmap(
                  hps,                 /* Handle to the Presentation space.   */
                  hbmBitmapToShow,     /* Handle to the bell bitmap.          */
                  (PRECTL) NULL,       /* Use all of the bitmap.              */
                  &ptlBitMapPoint,     /* The point to place the bitmap.      */
                  CLR_NEUTRAL,         /* The Foreground color.               */
                  CLR_BACKGROUND,      /* The background color.               */
                  DBM_NORMAL );        /* How to draw the bitmap.             */
            }
            else
            {
               WinDrawBitmap(
                  hps,                               /* Handle to the PS.     */
                  ahbmBellBitMaps[ IX_CENTER_BELL ], /* Center bell handle.   */
                  (PRECTL) NULL,       /* Use all of the bitmap.              */
                  &ptlBitMapPoint,     /* The point to place the bitmap.      */
                  CLR_NEUTRAL,         /* The Foreground color.               */
                  CLR_BACKGROUND,      /* The background color.               */
      ...
      case WM_ACTIVATE:
         ...
         if ((BOOL)mp1 && fPassed == TRUE) {
            ...
            dwError = mciSendCommand( mciOpenParameters.wDeviceID,
                                      MCI_ACQUIREDEVICE,
                                      (DWORD)MCI_NOTIFY,
...
VOID DealWithChiming( VOID )
{
           ...
           fThreadIsAlive == TRUE )
   {
      ...
      {
         yngContinueToSwingTheBell = READY;

         hbmBitmapToShow = ahbmBellBitMaps[ IX_CENTER_BELL ];

         WinInvalidateRect(
            hwndMainDialogBox,      /* Handle to the dialog box.              */
            &rctBitMapRectangle,    /* The area in the Pres. Space to update. */
            TRUE  );                /* Do not bother child window.            */
                                 ...
                                 USHORT             usHour )
{
   ...
   switch( ctvChimeTime )
   {
      case HOUR :
      {
          ...
          */
         mciOpenParameters.dwCallback = (DWORD)hwndMainDialogBox;
         ...
         mciSendCommand(
            ...
            (DWORD) &mciOpenParameters,   /* Parameters for the message.   */
...
VOID SetupChimeFileInformation( VOID )
{
   ...
   ulRC =
      mciSendCommand(
         mciOpenParameters.wDeviceID,
         MCI_SET,
         MCI_WAIT | MCI_WAVE_SET_CHANNELS,
         (DWORD) &mwspWaveFormParameters,
         0 );
...

{
    ...
    * message to MCI.
    */
   mciOpenParameters.lpstrDeviceType  = (LPSTR)
      MAKEULONG ( MCI_DEVTYPE_WAVEFORM_AUDIO, 1 );

   /*
    * The address of the buffer containing the chime waveform file.
    */
   mciOpenParameters.lpstrElementName =
      (LPSTR) &apltPlayList[ usChimePlaylistToPlay ][ 0 ];
    ...
    */
   mciOpenParameters.dwCallback       = (DWORD) hwndMainDialogBox;
   mciOpenParameters.lpstrAlias       = (CHAR) NULL;
...
VOID SwingTheBell( VOID )
{
             ...
             SEM_INDEFINITE_WAIT ) ==  NO_ERROR )
   {
      for( usCount=0; usCount<NUMBER_OF_BELLS_IN_A_SWING; usCount++ )
      {
        ...
        hbmBitmapToShow = ahbmBellBitMaps[ ausBellSwingOrder[ usCount ] ];
        ...
        DosSleep( PAUSE_BETWEEN_BITMAPS );
