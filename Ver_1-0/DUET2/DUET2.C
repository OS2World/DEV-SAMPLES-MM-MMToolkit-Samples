...
#pragma linkage( HelpWindowProc, system)
MRESULT EXPENTRY  HelpWindowProc( HWND   hwnd,
                                  ULONG  msg,
                                  MPARAM mp1,
                                  MPARAM mp2 );

#pragma linkage( AudioHelpWindowProc, system)
                                 ...
                                 MPARAM mp2 )
{
   ...
   switch( msg )
   {
      ...
      case WM_ACTIVATE:
          ...
          * we check to see if we've passed control of one of the
          * devices that we use.  If these conditions are true, then
          * we issue an acquire device command to regain control of
          * the device, since we're now the active window on the screen.
          */

         /*
          * First we check to see if we've passed control of a Duet.
          */
...
BOOL PlayTheDuet( HWND hwnd)
{
   ...
   if ( DoesFileExist( aDuet[sDuet].achPart2))
   {
       ...
       */
      {
         ...
         mopDuetPart.lpstrDeviceType  = (LPSTR) MCI_DEVTYPE_CD_AUDIO;
         mopDuetPart.lpstrElementName = (LPSTR) NULL; /* doesn't matter       */
         ...
         if (!dwError)  /* if we opened part 1 */
         {
            ...
            mspCD.dwItem = MCI_STATUS_MEDIA_PRESENT;
            ...
            else if (mspCD.dwReturn == TRUE) /* if we have a CD */
            {
               /*
                * OK, we have a CD.  Now, open the other part
                */
               mopDuetPart.lpstrDeviceType  = (LPSTR) NULL;
       ...
       */
      {
          ...
          */
          ...
          if (mspCD.dwReturn == TRUE) /* if we have a CD */
          {
             /*
              ...
              */
             mlpLoad.dwCallback       = (DWORD) NULL;
             mlpLoad.lpstrElementName = (LPSTR) aDuet[sDuet].achPart2;
      ...
      if (dwError==MCIERR_SUCCESS)  /* if we made the group */
      {
           ...
           * message will be sent to the window specified in dwCallback when
      ...
      case UM_PLAY_AUDIO_HELP:
         if (!fPlayingAudioHelp)
         {
            ...
            if ( DoesFileExist( achAudioHelpFile))
            {
               ...
               mciOpenParms.wDeviceID        = (WORD) NULL;
               mciOpenParms.lpstrDeviceType  = (LPSTR) NULL;
