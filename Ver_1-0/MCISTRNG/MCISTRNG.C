 ...
 *               MCISTRNG sample program.  The user enters MCI String
 *               commands from a combination box and presses the Send
 *               pushbutton.  The command is sent to the MCD and the string
 *               is placed in the combination box's list box.
 *
 ...
 *               MCI Notification messages that are recieved by the Call
 *               Back window.
 *
 ...
 *               mcistr1.ipf        Help text for the Main Dialog.
 *               mcistr2.ipf        Help text for the Display Messages Dialog.
 *               mcistr3.ipf        Help text for the Include Dialog.
 *               makefile           Make file.
...
VOID             InitializeHelp( HELPINIT       *phmiHelpStructure,
                                 WHICH_DIALOG_T wdtDialog );
...
HELPINIT hmiMainHelpStructure,         /* Help initialization structures.     */
         hmiIncludeHelpStructure,
         hmiDisplayHelpStructure;
...
BOOL  fDisplayMessageWindowIsUp = FALSE;  /* flag for display message window  */
/*
...
HWND   hwndMainDialogBox,              /* Handle to the Main Dialog window.   */
       ...
       hwndMainHelpInstance,           /* Handle to the Main Help window.     */
       hwndIncludeHelpInstance,        /* Handle to the Include Help window.  */
       hwndDisplayHelpInstance,        /* Handle to the Display Help window.  */
...
VOID InitializeDialog( VOID )
{
   ...
   /*
    * Now that the Display Message dialog box has been created, show the
    * Main Dialog window.
    */
   WinShowWindow(
      hwndFrame,
   ...
   /*
    * Initialize the help structures and associate the help instances to the
    * correct dialogs via the handle to anchor block.
    */
   InitializeHelp(
      &hmiMainHelpStructure,     /* Structure to hold help information.       */
      MAIN_DIALOG );             /* Which dialog is being initialized.        */

   InitializeHelp(
      &hmiDisplayHelpStructure, /* Structure to hold help information.        */
      DISPLAY_DIALOG );         /* Which dialog is being initialized.         */

   InitializeHelp(
      &hmiIncludeHelpStructure, /* Structure to hold help information.        */
      INCLUDE_DIALOG );         /* Which dialog is being initialized.         */
...
VOID TerminateDialog( VOID )
{
    ...
    */
   if ( hwndMainHelpInstance != (HWND) NULL )
   {
      WinDestroyHelpInstance( hwndMainHelpInstance );
   }

   if ( hwndDisplayHelpInstance != (HWND) NULL )
   {
      WinDestroyHelpInstance( hwndDisplayHelpInstance );
   }

   if ( hwndIncludeHelpInstance != (HWND) NULL )
   {
      WinDestroyHelpInstance( hwndIncludeHelpInstance );
   }
   ...
   switch( msg )
   {
      ...
      case WM_COMMAND :
         ...
         switch( SHORT1FROMMP( mp1 ) )
         {
            ...
            case ID_DISPLAY_MESSAGES_PB :
               ...
               fDisplayMessageWindowIsUp = TRUE; /* Visible, set the flag.    */
      ...
      case WM_MINMAXFRAME :
      {
         ...
         if ( pswpWindowActionMP->fl & SWP_MINIMIZE )
         {
            ...
            */
           if (fDisplayMessageWindowIsUp == TRUE)
              WinShowWindow(
                 hwndDisplayDialogBox,
                 FALSE );
                                    ...
                                    MPARAM mp2 )
{
   ...
   switch( msg )
   {
      ...
      case WM_INITDLG :
         ...
         fDisplayMessageWindowIsUp = FALSE;   /* Not visible, set the flag.   */
      ...
      case MM_MCINOTIFY :
         ...
         if ( afCheckBoxState[ ID_MM_MCINOTIFY ] == TRUE )
         {
             ...
             * we also want to display the information recieved with the
 ...
 *               proceedure will trap all appropriate keyboard and mouse
 *               actions so that no selections can be made.  All other
 *               messages received by this function that do not invlove
 *               the relevent mouse or keyboard actions are passed back to
                                     ...
                                     MPARAM mp2 )
{
   ...
   switch( msg )
   {
      ...
      case WM_BUTTON1DOWN :
      {
         ...
         /*
          * See if the mouse is in the forbbiden.  If it is, ignore the
          * mouse action.
          */
...
VOID SendStringToMCD( HWND hwndComboBox )
{
   static USHORT usCountOfMCIStringSent = 1;            /* Number of sent cmds*/
   CHAR   acTempString[ STRING_SIZE ];                  /* Temp string buffer.*/

   CHAR acMCIString[ MCI_STRING_LENGTH ],               /* MCI String Command.*/
        acErrorStringBuffer[ MCI_ERROR_STRING_LENGTH ]; /* Error string buffer*/
   LONG lSendStringRC     = 0,                          /* Send String API RC.*/
   ...
   lSendStringRC =
      mciSendString(
         (LPSTR) &acMCIString[ 0 ],       /* The MCI String Command.          */
         (LPSTR)&acMCIReturnString[ 0 ],  /* The place to put Return strings. */
         (WORD) MCI_RETURN_STRING_LENTGH, /* How large is the Return space.   */
         hwndDisplayDialogBox,            /* Which window recieves Notifies.  */
   ...
   if ( lSendStringRC != 0 )
   {
      ...
      lGetErrorStringRC =
         mciGetErrorString(
            lSendStringRC,
            (LPSTR) &acErrorStringBuffer[ 0 ],
            (WORD) MCI_ERROR_STRING_LENGTH );
      ...
      WinMessageBox(
         ...
         MB_CANCEL | MB_MOVEABLE | MB_APPLMODAL | MB_HELP |
 ...
 * MMPM/2 API's: None.
 *
 * Parameters  : phmiHelpStructure - Help structure for a particular dialog.
 *               wdtDialog         - Which Dialog box to setup.
 *
 * Return      : None.
 *
 ******************************************************************************/
                     ...
                     WHICH_DIALOG_T wdtDialog )
{
    ...
    */
   phmiHelpStructure->cb              = sizeof( HELPINIT );

   phmiHelpStructure->ulReturnCode    = (USHORT) NULL; /* RC from HM initial. */
   phmiHelpStructure->pszTutorialName = (PSZ) NULL;    /* No tutorial program.*/

   /*
    * The action bar is not used, so set the following to NULL.
    */
   phmiHelpStructure->hmodAccelActionBarModule = (HMODULE) NULL;
   phmiHelpStructure->idAccelTable             = (USHORT) NULL;
   phmiHelpStructure->idActionBar              = (USHORT) NULL;

   /*
    * The Help table is in the executable file.
    */
   phmiHelpStructure->hmodHelpTableModule = (HMODULE) NULL;

   /*
    * The help panel ID is not displayed.
    */
   phmiHelpStructure->fShowPanelId       = (ULONG) NULL;

   /*
    * Setup the help based on the wdtDialog value passed into this function.
    */
   switch( wdtDialog )
   {
      case MAIN_DIALOG :
      {
          ...
          */
         phmiHelpStructure->pszHelpLibraryName  =
            acStringBuffer[ IDS_MAIN_HELP_LIBRARY_FILE - 1 ];
         /*
          * The Help window title.
          */
         phmiHelpStructure->pszHelpWindowTitle  =
            acStringBuffer[ IDS_MAIN_HELP_WINDOW_TITLE - 1 ];

         phmiHelpStructure->phtHelpTable    = (PVOID)(0xffff0000 |
            ID_MCI_STRING_MAIN_DIALOG_HELPTABLE);
         ...
         hwndMainHelpInstance =
            WinCreateHelpInstance(
               ...
               phmiHelpStructure );  /* Help Structure.                       */
         ...
         {
            if ( phmiHelpStructure->ulReturnCode )
            {
               ShowAMessage(
                  hwndMainDialogBox,                 /* Owner Window          */
                  acStringBuffer[ IDS_NORMAL_ERROR_MESSAGE_BOX_TEXT - 1 ],
         ...
         if ( hwndMainHelpInstance != (HWND) NULL )
         {
           WinAssociateHelpInstance(
              hwndMainHelpInstance, /* The handle of the Help Instance.       */
              hwndFrame );          /* Associate to this dialog window.       */
         }

         break;

      }  /* End of case MAIN_DIALOG. */
      ...
      case INCLUDE_DIALOG :
      {
          ...
          */
         phmiHelpStructure->pszHelpLibraryName  =
            acStringBuffer[ IDS_INCLUDE_HELP_LIBRARY_FILE - 1 ];

         /*
          * The Help window title.
          */
         phmiHelpStructure->pszHelpWindowTitle  =
            acStringBuffer[ IDS_INCLUDE_HELP_WINDOW_TITLE - 1 ];

         phmiHelpStructure->phtHelpTable    = (PVOID)(0xffff0000 |
            ID_MCI_STRING_INCLUDE_DIALOG_HELPTABLE);
         ...
         if ( hwndIncludeHelpInstance == (HWND) NULL )
         {
            ShowAMessage(
               hwndMainDialogBox,                    /* Owner Window          */
               acStringBuffer[ IDS_NORMAL_ERROR_MESSAGE_BOX_TEXT - 1 ],
               IDS_INCLUDE_HELP_CREATION_FAILED,     /* ID of the message.    */
               MB_APPLMODAL | MB_MOVEABLE |
                  MB_OK | MB_INFORMATION );          /* Message box style.    */
         }
         else
         ...
         {
           WinAssociateHelpInstance(
              hwndIncludeHelpInstance, /* The handle of the Help Instance.    */
              hwndIncludeDialogBox );  /* Associate to this dialog window.    */
         }

         break;

      }  /* End of case INCLUDE_DIALOG. */
      ...
      case DISPLAY_DIALOG :
      {
          ...
          */
         phmiHelpStructure->pszHelpLibraryName  =
            acStringBuffer[ IDS_DISPLAY_HELP_LIBRARY_FILE - 1 ];

         /*
          * The Help window title.
          */
         phmiHelpStructure->pszHelpWindowTitle  =
            acStringBuffer[ IDS_DISPLAY_HELP_WINDOW_TITLE - 1 ];

         phmiHelpStructure->phtHelpTable    = (PVOID)(0xffff0000 |
            ID_MCI_STRING_DISPLAY_DIALOG_HELPTABLE);
         ...
         if ( hwndDisplayHelpInstance == (HWND) NULL )
         {
            ShowAMessage(
               hwndMainDialogBox,                    /* Owner Window          */
               acStringBuffer[ IDS_NORMAL_ERROR_MESSAGE_BOX_TEXT - 1 ],
               IDS_DISPLAY_HELP_CREATION_FAILED,     /* ID of the message.    */
               MB_APPLMODAL |  MB_MOVEABLE |
                  MB_OK | MB_INFORMATION );          /* Message box style.    */
         }
         else
         ...
         {
           WinAssociateHelpInstance(
              hwndDisplayHelpInstance, /* The handle of the Help Instance.    */
              hwndDisplayDialogBox );  /* Associate to this dialog window.    */
         }

         break;

      }  /* End of case DISPLAY_DIALOG. */

   }  /* End of switch on Dialog Box. */
                     ...
                     ULONG  ulMessageBoxStyle )
{
   ...
   CHAR   acStringBuffer[ STRING_SIZE ];        /* Buffer for RC string.      */
