 ...
 *                 One part will be streamed from a waveform file and the other
 *                 part will be played from an audio CD.  Also, an audio help
 *                 message will be incorporated into IPF help.
 *
 ...
 *
 * Required
 *    Files     :  duet2.c        Source Code.
 *                 duet2.h        Include file.
 *                 duet2.dlg      Dialog definition.
 *                 duet2.rc       Resources.
 *                 duet2.ipf      Help text.
 *                 duet2.mak      Make file.
 *                 duet2.def      Linker definition file.
 *                 duet2.ico      Program icon.
 *
...
#define  INCL_MACHDR                /* required for mciplayfile            */
 ...
 * you open the device, you specify if you want exclusive use of the device,
 * or if you're willing to share the device.  The Duet Player opens all of
 * its devices as shareable devices.  The MMPM/2 default is to open the
 * device exclusively unless otherwise specified by the app.
 *
 ...
 * between the time we open it and the time we play it.  Also, logic could
 * be added to make sure that another application doesn't have exclusive
 * use of the device when we open it.  The current implementation in the
 * Duet Player will simply display an error message to the user when one
 * of these two situations exist.
 *
 *************************************************************************/
BOOL PlayTheDuet( HWND hwnd)
{
   ...
   MCI_STATUS_PARMS  mspCD;                  /* for checking status of CD     */
    ...
    */
   if ( DoesFileExist( aDuet[sDuet].achPart2))
   {
       ...
       */
      {
          ...
          * We will be using an open with only the device type specified.
          * This will use whatever disc happens to be in the CD.
          */
         mopDuetPart.hwndCallback       = hwnd;         /* For MM_MCIPASSDEVICE */
         mopDuetPart.usDeviceID       = (USHORT)  NULL; /* this is returned   */
         mopDuetPart.pszDeviceType    = (PSZ)   MCI_DEVTYPE_CD_AUDIO;
         mopDuetPart.pszElementName   = (PSZ)   NULL; /* doesn't matter       */
         ...
         if (!ulError)  /* if we opened part 1 */
         {
            ...
            /*
             * We have opened the CD, now we need to be sure that media is
             * present in the player.
             */

            mspCD.ulItem = MCI_STATUS_MEDIA_PRESENT;
            ...
            else if (mspCD.ulReturn == TRUE) /* if we have a CD */
            {
               /*
                * We have the CD, now we need to be sure that media is
                * playable.
                */

               mspCD.ulItem = MCI_STATUS_NUMBER_OF_TRACKS;
               ...
               else if (mspCD.ulReturn != (USHORT) NULL)  /* if CD is playable  */
               {
                  /*
                   * OK, we have a playable CD.  Now, open the other part
                   */
                  mopDuetPart.pszDeviceType    = (PSZ)   NULL;
                  ...
                  if (!ulError)  /* if we opened part 2 */
                  {
                     ...
                     /*
                      * So far, so good.  Now we need to create a group.  To do
                      * this, we need to fill an array with the ID's of the
                      * already open devices that we want to group.  Then we
                      * call MCI_GROUP to create the group and return us a
                      * handle to it.
                      */
                     ...
                     fFirstPlay = FALSE;               /* opened the devices  */
      ...
      {
         /*
          * We need to be sure that media is still present in the player.
          */
          mspCD.ulItem = MCI_STATUS_MEDIA_PRESENT;
          ...
          if (mspCD.ulReturn == TRUE) /* if we have a CD */
          {
             ...
             {
                /*
                 * OK, we have a CD. Load the other part of the duet. The first
                 * step is to initialize an MCI_LOAD_PARMS data structure with
                 * the appropriate information, then issue the MCI_LOAD command
                 * with the mciSendCommand function.
                 */
                mlpLoad.hwndCallback       = (HWND) NULL;
                mlpLoad.pszElementName   = (PSZ)   aDuet[sDuet].achPart2;
      ...
      if (ulError==MCIERR_SUCCESS)
      {
          ...
          /*
           * OK, now we need to start playing. First we need to initialize
           * an MCI_PLAY_PARMS structure with the pertinent information then
           * issue an MCI_PLAY command via mciSendCommand.  This time we will
           * be using the notification version of the command - a MM_MCINOTIFY
           * message will be sent to the window specified in hwndCallback when
           * the operation is completed.  Since we want the whole audio file
           * to be played, we won't specify flags of MCI_FROM or MCI_TO so we
           * don't need to fill in these parameters in the structure.
           */
