...
VOID    DisplaySaveasDlg(SHORT sMenuItem);
        ...
        switch ( sCommandID )
        {
           ...
           case IDM_OPEN_TEXT:
              ...
              if (WinFileDlg(HWND_DESKTOP, hwnd, &fdgText))/*(4) Ask for name */
                 ...
                 {
                    if ( iState == ST_PLAYING ) StopPlaying( hwnd );
                    if ( apiret = LoadTextFile ( fdgText.szFullFile ) )
...
/*---------------- Write the Caption data to the file ------------------------*/
   ...
   if ( ! ( apiret = WriteLine ( hfile, (PSZ) szCC_FILE ) ) )
   {
      pld = (PLINEDATA) pvLinedataTable;
      for ( usLineIndex = 0; usLineIndex < usLineCount; usLineIndex++ )
      {
         ...
         if ( apiret = WriteLine ( hfile, (PSZ) szOutput ) ) break;
         pld++;
      }
      if ( ! apiret )
      {
...
/*----------------------------------------------------------------------------*/
   ...
   WinQueryDefaultSize ( hwndMainFrame, &swpDefault ) ;
...
/*----------------------------------------------------------------------------*/
   ...
   if ( lModulo = lLineSpacing % CC_SCROLL_INC ) /* (3) Adjust spacing to int */
